...
EBTNodeResult::Type UFindRandomLocation::ExecuteTask(UBehaviorTreeComponent& owner_comp, uint8* node_memory)
{
	...
	auto const spidering = cont->GetPawn();

	//obtain npc location to use as an origin location
	FVector const origin = spidering->GetActorLocation();
	FNavLocation loc;

	//get the navigation system and generage a random location on the NavMesh
